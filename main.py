        ...
        for cpu_id in self.eventsByCpuId:
            for name in self.eventsByCpuId[cpu_id]:
                if name in eventsByName:
                    eventsByName[name]["entries"] = eventsByName[name]["entries"] + self.eventsByCpuId[cpu_id][name][
                        "entries"]
                    eventsByName[name]["exits"] = eventsByName[name]["exits"] + self.eventsByCpuId[cpu_id][name][
                        "exits"]
                    eventsByName[name]["spans"] = eventsByName[name]["spans"] + self.eventsByCpuId[cpu_id][name][
                        "spans"]
                else:
                    eventsByName[name] = {
...
msg_it = bt2.TraceCollectionMessageIterator("venv/traces/kernel")
